  ...
  initSignupForm() {
    this.usersService.getUser(this.id).subscribe((data) => {
      ...
      this.signupForm = this.formBuilder.group({
        ...
        birthDate: [new Date(data.birthDate), [Validators.required, SignUpValidator.validDate]], // birthdat should be in the past
      });
      this.loading = false;
    });
  }
  ...
  onSubmitSignupForm() {
    ...
    this.usersService.editUser(user).subscribe(
      (response: any) => {
        ...
        this.alertSignup = { type: 'success', message: 'User Modified successfully' };
      },
      (error) => {
        // console.log(error);
